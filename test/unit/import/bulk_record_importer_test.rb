...
class BulkRecordImporterTest < Minitest::Test
  ...
  def setup
    HealthDataStandards::Record.all.delete
  end
  ...
  def test_bulk_record_import_xml_archive
    ...
    patient1 = HealthDataStandards::Record.where({"first" => "Cancer_Adult_Female"}).first
    patient2 = HealthDataStandards::Record.where({"first" => "Cancer_Adult_Male"}).first
 ...
 def test_bulk_record_import_json_archive
  ...
  def test_bulk_record_import_bad_archive
    ...
    assert !err.nil?
    assert_equal 0, HealthDataStandards::Record.count
